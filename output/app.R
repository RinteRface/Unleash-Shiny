...
boxxyOutput <- function(id){
  el <- shiny::tags$div(
    id = id, class = "box boxxy",
    div(
      class = "level-item has-text-centered",
      div(
        h1(id = sprintf("%s-boxxy-value", id), class = "heading boxxy-value"),
        p(id = sprintf("%s-boxxy-title", id), class = "title boxxy-title")
      )
    )
  )
...
ui <- bulmaPage(
  bulmaContainer(
    bulmaColumns(
      bulmaColumn(boxxyOutput("countries")),
      bulmaColumn(boxxyOutput("employees")),
      bulmaColumn(boxxyOutput("customers")),
      bulmaColumn(boxxyOutput("subs"))
    )
  )
)
