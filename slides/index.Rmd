...
The grand scheme: Serve data as an __HTTP response__ and read it in JavaScript to produce the __output.__ 
...
- `Status` - How is the response? 
  ...
  - `301` redirect
- `Content-Type` - type of content (HTML, JSON, etc.)
...
---
class: header_background
# A.1.3.1 HTTP response __name__
...
---
class: header_background
# A.1.3.2 HTTP response __Data__

```r
path <- session$registerDataObj(name = "about", data = "<h1>Hello!</h1>", filterFunc)
```

The `data` is simply passed to `filterFunc` (up next).

It can be pretty much any object you like; `data.frame`, `list`, a `character` string, etc. 
...
<p class="rstudio-comment">Open <code>custom-output/app.R</code></p>
